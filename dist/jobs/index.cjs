...
var ApifyProvider = class {
  ...
  async runInstagramProfilePostScraper(profile) {
    const { data } = await import_axios2.default.post(env2.APIFY_BASE_URL + `/acts/apify~instagram-post-scraper/runs`, {
      "username": [profile],
      "resultsLimit": 500,
  ...
  async getDatasetById(id) {
    const { data } = await import_axios2.default.get(`https://api.apify.com/v2/datasets/${id}/items`);
    return data;
  }
};
